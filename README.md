# Hi, I'm Abhishek Tiwari 👋

## Senior Java Developer | Technical Lead | Spring Boot | AWS Certified

I'm a passionate Java Backend Developer with **7+ years of experience** designing, developing, modernizing, and supporting enterprise applications across the **Insurance, BFSI, Risk & Compliance, and Government domains**.

I specialize in building scalable backend systems using **Java**, **Spring Boot**, **REST APIs**, **Microservices**, and **AWS**, with hands-on experience in cloud migration, production support, CI/CD automation, and enterprise application modernization.

---

## 💼 Professional Experience

- 🏢 **Senior Technical Lead** – HCLTech *(Current)*
- 🏢 **Senior Analyst Software Engineering** – DXC Technology
- 🏢 **Full Stack Java Developer** – EPFO (Nelito Systems Ltd. & Dev Information Technology Pvt. Ltd.)

---

## 🚀 What I Work On

- Enterprise Backend Development
- REST API Design & Development
- Spring Boot & Microservices
- IBM WebSphere & Apache Tomcat
- Production Support & Root Cause Analysis
- CI/CD Pipeline Automation
- AWS Cloud Migration
- Security Vulnerability Remediation
- Rule Engine Development using Drools
- Automated Testing using TestNG
- Performance Optimization
- Code Reviews & Technical Mentoring

---

## 🛠️ Tech Stack

### Backend
- Java 8 | Java 11 | Java 17
- Spring Boot
- Spring Framework
- Spring MVC
- Spring Data JPA
- Hibernate
- MyBatis
- REST APIs
- Microservices
- Drools Rule Engine

### Database
- MySQL
- DB2
- Oracle

### Cloud & DevOps
- AWS EC2
- CodeCommit
- CodeBuild
- CodeDeploy
- CodePipeline
- Jenkins
- Git
- GitHub
- GitLab
- Maven

### Security
- JWT
- OAuth 2.0
- LDAP
- Swagger / OpenAPI
- Vulnerability Remediation

### Testing
- TestNG
- JUnit

### Servers
- IBM WebSphere
- Apache Tomcat

### Tools
- Eclipse
- IntelliJ IDEA
- Postman
- JIRA

### AI & Productivity
- GitHub Copilot
- OpenAI Codex
- Prompt Engineering
- AI Agents

---

## 🏆 Certifications

✅ AWS Certified Developer – Associate

✅ AWS Certified AI Practitioner

✅ AWS Certified Cloud Practitioner

---

## 📌 Current Focus

- Enterprise Java Development
- Spring Boot & Microservices
- AWS Cloud Solutions
- Backend System Design
- AI-assisted Software Development
- Building High-Quality Developer Tools

---

## 🌐 Portfolio

🔗 https://abhishekhartiwari.github.io/portfolio/

---

## 📫 Connect With Me

📧 **Email**
abhishekhartiwari@gmail.com

💼 **LinkedIn**
https://linkedin.com/in/abhishekhartiwari

💻 **GitHub**
https://github.com/abhishekhartiwari

🌐 **Portfolio**
https://abhishekhartiwari.github.io/portfolio/

---


## 📊 GitHub Stats
![Abhishek's GitHub Stats](https://github-readme-stats.vercel.app/api?username=abhishekhartiwari&show_icons=true&theme=github_dark&hide_border=true)
## 💻 Most Used Languages
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=abhishekhartiwari&layout=compact&theme=github_dark&hide_border=true)
## 🔥 Contribution Streak
[![GitHub Streak](https://streak-stats.demolab.com?user=abhishekhartiwari&theme=github-dark&hide_border=true)](https://git.io/streak-stats)
## 📈 Contribution Graph
[![Abhishek's Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=abhishekhartiwari&theme=github-dark)](https://github.com/ashutosh00710/github-readme-activity-graph)
## 👀 Profile Views
![](https://komarev.com/ghpvc/?username=abhishekhartiwari&label=Profile%20Views&color=0e75b6&style=flat)
## 🏆 GitHub Trophies
[![Trophies](https://github-profile-trophy.vercel.app/?username=abhishekhartiwari&theme=onedark&margin-w=15)](https://github.com/ryo-ma/github-profile-trophy)
## ⚙️ Technologies
<p>
<img src="https://skillicons.dev/icons?i=java,spring,mysql,aws,git,maven,github,jenkins,hibernate,postman,eclipse,vscode" />
</p>

## 💡 Quote

> "Clean code, scalable architecture, continuous learning, and delivering business value through technology."
